<?php
  ...
  if (isset($_POST['submitted']) Or isset($_POST['h_sys_submitted']))
  {
    ...
    {
      $my_error .= "Solar arc date - please enter a year between 1200 and 2399.<br>";
    }
...
//get house positions of planets here - solar arc planets in natal houses
      for ($x = 1; $x <= 12; $x++)
      {
        for ($y = 0; $y <= LAST_PLANET; $y++)
        {
...
// no need to urlencode unless perhaps magic quotes is ON (??)
    ...
    $_SESSION['house_pos1_sa'] = $house_pos1;
    $_SESSION['house_pos2_in_1_sa'] = $house_pos2_in_1;
    $_SESSION['house_pos3_in_1_sa'] = $house_pos3_in_1;
    
    $_SESSION['solar_arc'] = $solar_arc;


    echo "<center>";
    echo "<img border='0' src='transits_solar_arcs_natal_wheel.php' width='930' height='930'>";

    echo "<br /><br />";
    echo "SOLAR ARCS<br />";
    echo "<img border='0' src='sa_aspect_grid_for tr_sa.php?rx1=$rx1&rx2=$rx2&sa=$solar_arc&ubt1=$ubt1&ubt2=$ubt2' width='830' height='475'>";

    echo "<br />TRANSITS<br />";
    echo "<img border='0' src='transit_aspect_grid_blue_SA.php?rx1=$rx1&rx2=$rx3&ubt1=$ubt1&ubt2=$ubt3' width='830' height='475'>";
...
// display transit data - aspect table
    ...
    // update count
    $sql = "SELECT solar_arcs_and_transits FROM astro_reports";
    $result = @mysqli_query($conn, $sql) or error_log(mysqli_error($conn), 0);
    $row = mysqli_fetch_array($result);
    $count = $row[solar_arcs_and_transits] + 1;

    $sql = "UPDATE astro_reports SET solar_arcs_and_transits = '$count'";
    $result = @mysqli_query($conn, $sql) or error_log(mysqli_error($conn), 0);
