...
//$adataJson = file_get_contents("http://astro.slider.com/online_calcs/scripts/right_now_JSON.php");
...
for ($i = $num_planets - 1; $i >= 0; $i--) {
    if ($sort_pos[$i] == SE_POF Or $sort_pos[$i] == SE_VERTEX) {
        continue;
    }
...
// run through all the planets and see how many planets are in each house
    for ($i = 0; $i <= $num_planets - 1; $i++) {
        if ($sort_pos[$i] != SE_POF And $sort_pos[$i] != SE_VERTEX) {
