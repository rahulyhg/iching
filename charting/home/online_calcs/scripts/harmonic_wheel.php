<?php
  ...
  $longitude = $_SESSION['longitude2_HA'];
  $hc = $_SESSION['hc2_HA'];
  $house_pos = $_SESSION['house_pos2_HA'];
  
  $longitude2_MC = $_SESSION['longitude2_MC'];          //"floating" MC

  $longitude[LAST_PLANET + 1] = $hc[1];
  $longitude[LAST_PLANET + 2] = $longitude2_MC;
...
// specify the colors
  ...
  $another_blue = imagecolorallocate($im, 212, 235, 242);
...
// specific colors
  ...
  if ($ubt1 == 0)                         //include the Asc and "floating" MC
  {
...
// create colored rectangle on blank image
  imagefilledrectangle($im, 0, 0, $size_of_rect, $size_of_rect + $y_top_margin, $background_color);     //was $white
...
// draw the outer-outer border of the chartwheel
  imagefilledellipse($im, $center_pt_x, $center_pt_y, $outer_outer_diameter + 40, $outer_outer_diameter + 40, $another_blue);
...
//data for chart
  imagettftext($im, 8, 0, 10, 38, $black, './arial.ttf', $line1);

  imagettftext($im, 8, 0, 450, 675, $black, './arial.ttf', $copyright1);
  imagettftext($im, 8, 0, 450, 688, $black, './arial.ttf', $copyright2);
...
// put planets in chartwheel
  ...
  Sort_planets_by_descending_longitude($num_planets, $longitude, $house_pos, $sort, $sort_pos);

  // count how many planets are in each house
  Count_planets_in_each_house($num_planets, $house_pos, $sort_pos, $sort, $nopih);
  ...
  for ($i = $num_planets - 1; $i >= 0; $i--)
  {
    ...
    if ($temp != $house_num) { $planets_done = 1; }      // this planet is in a different house than the last one - this planet is the first one in this house, in other words
    ...
    while (Check_for_overlap($angle, $spot_filled, $spacing) == True) { $angle = Crunch($angle + 1); }
...
Function Sort_planets_by_descending_longitude($num_planets, $longitude, $house_pos, &$sort, &$sort_pos)
{
...
Function Count_planets_in_each_house($num_planets, $house_pos, &$sort_pos, &$sort, &$nopih)
{
