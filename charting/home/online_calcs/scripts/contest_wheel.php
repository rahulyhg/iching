...
//start the mutual reception listing here
...
//mutual receptions - by sign
  ...
  {
    $t = floor($longitude[$i] / 30) + 1;      //get the sign of the first planet

    if ($t == 1 Or $t == 8) { $x = SE_MARS; }
    if ($t == 2 Or $t == 7) { $x = SE_VENUS; }
    if ($t == 3 Or $t == 6) { $x = SE_MERCURY; }
    if ($t == 4) { $x = SE_MOON; }
    if ($t == 5) { $x = SE_SUN; }
    if ($t == 9 Or $t == 12) { $x = SE_JUPITER; }
    if ($t == 10 Or $t == 11) { $x = SE_SATURN; }

    $t = floor($longitude[$x] / 30) + 1;      //get the sign of the second planet
...
//mutual receptions - by exaltation
  for ($i = SE_SUN; $i <= SE_SATURN; $i++)
  {
    $t = floor($longitude[$i] / 30) + 1;      //get the sign of the first planet

    if ($t == 3 Or $t == 5 Or $t == 8 Or $t == 9 Or $t == 11) { continue; }
...
//mutual receptions - by sign/exaltation
  for ($i = SE_SUN; $i <= SE_SATURN; $i++)
  {
    $t = floor($longitude[$i] / 30) + 1;      //get the sign of the first planet
