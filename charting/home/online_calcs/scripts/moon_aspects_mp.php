<?php
  ...
  Moon_Aspectarian($pl_name, $start_month, $start_day, $start_year, $num_days, $timezone2, $pl_ephem_number, $aspects_defined);
...
Function Moon_Aspectarian($pl_name, $init_month, $init_day, $init_year, $num_days, $timezone, $pl_ephem_number, $aspects_defined)
{
  ...
  echo "<table align='center' width='98%'><tr><td><font size='2'>";
  ...
  for ($x = $starting_JD; $x <= $ending_JD; $x++)
  {
    ...
    for ($q = 1; $q <= 7; $q++)         //$q is aspect number
    {
      if ($q != 2 And $q != 6)
      {
        ...
        $secant_results = Secant_Method_MOON($x, $x + 1, 0.00007, 100, $angle, $p1_idx, $p2_idx, $p2);
        $Result_JD = $secant_results[0];

        $rnd_off = $Result_JD;
        if ($rnd_off >= $x And $rnd_off < $x + 1)
        {
          $first_date = ConvertJDtoDateandTime_MOON($Result_JD, $current_tz);
...
Function Secant_Method_MOON($earlier_jd, $later_jd, $e, $m, $angle, $p1_idx, $p2_idx, $p2)
{
  for ($n = 1; $n <= $m; $n++)
  {
    //get positions of both planets on JD = later_jd and JD = earlier_jd
    $result = Get_2_Planets_geo_MOON($later_jd, $p1_idx, $p2_idx);
    $y1 = $result[0];
    $speed1 = $result[1];
    $y2 = $result[2];
    $speed2 = $result[3];

    $result = Get_2_Planets_geo_MOON($earlier_jd, $p1_idx, $p2_idx);
    $y3 = $result[0];
    $y4 = $result[2];
...
Function Get_1_Planet_geo_MOON($jd, $p_idx)
{
  ...
  unset($out,$t_long);
  exec ("swetest -edir$sweph -bj$jd -ut -p$p_idx -eswe -fls -g, -head", $out);
...
Function ConvertJDtoDateandTime_MOON($Result_JD, $current_tz)
{
...
//aspectarian for when a planet hits a certain degree of the zodiac
  ...
  {
    $angle = 0;
    $secant_results = Secant_Method_one_degree_MOON($x, $x + 1, 0.00007, 100, $angle, $p1_idx, $degr);
    $Result_JD = $secant_results[0];
...
Function Secant_Method_one_degree_MOON($earlier_jd, $later_jd, $e, $m, $angle, $p1_idx, $degr)
{
  for ($n = 1; $n <= $m; $n++)
  {
    //get positions of both planets on JD = later_jd and JD = earlier_jd
    $ls = Get_1_Planet_geo_MOON($later_jd, $p1_idx);
    $y1 = $ls[0];
    $ls = Get_1_Planet_geo_MOON($earlier_jd, $p1_idx);
    $y3 = $ls[0];
...
Function Get_2_Planets_geo_MOON($jd, $p1_idx, $p2_idx)
{
  ...
  unset($out,$t_long);
  exec ("swetest -edir$sweph -bj$jd -ut -p$p1_idx$p2_idx -eswe -fls -g, -head", $out);
