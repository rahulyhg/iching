<?php
  ...
  $num_midpoint_aspects = $_SESSION['num_midpoint_aspects'];
  $nmp = $_SESSION['nmp'];
  $house_pos_nmp = $_SESSION['house_pos_nmp'];
  
  for ($i = 0; $i <= $num_midpoint_aspects - 1; $i++)
  {
    $longitude1[$i] = $nmp[2][$i];
  }
  ...
  $longitude2[LAST_PLANET + 1] = $hc1[1];
  $longitude2[LAST_PLANET + 2] = $hc1[10];
...
// put planets in chartwheel - natal midpoints in outer wheel
  // sort longitudes in descending order from 360 down to 0
  Sort_planets_by_descending_longitude($num_midpoint_aspects, $longitude1, $sort, $sort_pos);

  // count how many planets are in each house
  Count_planets_in_each_house($num_midpoint_aspects, $house_pos_nmp, $sort_pos, $sort, $nopih, $home);

  //find the best planet to start displaying the wheel and reorder the $sort[] array starting with that planet
  Find_best_planet_to_start_with($num_midpoint_aspects, $house_pos_nmp, $sort_pos, $sort, $nopih);
  ...
  for ($i = $num_midpoint_aspects - 1; $i >= 0; $i--)
  {
    ...
    $temp = $house_num;
    $house_num = $house_pos_nmp[$sort_pos[$i]];              // get the house this planet is in
    ...
    //find the two planets that make up this midpoint
    for ($ii = 0; $ii <= $num_midpoint_aspects - 1; $ii++)
    {
      if ($sort[$i] == $longitude1[$ii])
      {
        $p_first = $nmp[0][$ii];
        $p_second = $nmp[1][$ii];
        break;
    ...
    display_planet_glyph_MP_GLYPH($our_angle, $angle_to_use, $radius - $dist_from_diameter1, $xy);
    imagettftext($im, 9, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $planet_color, 'HamburgSymbols.ttf', chr($pl_glyph[$p_first]) . "/" . chr($pl_glyph[$p_second]));
    ...
    imagettftext($im, 10, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $planet_color, 'arial.ttf', $t . chr(176));    //deg symbol
...
// put planets in chartwheel - natal planets in inner wheel
  // sort longitudes in descending order from 360 down to 0
...
Function display_planet_glyph_MP_GLYPH($our_angle, $angle_to_use, $radii, &$xy)
{
// for planet glyph ONLY
// $our_angle in degree, $angle_to_use in radians
  $this_angle = Crunch($our_angle);
